...
# Exec name
EXEC_NAME=src/lanParty

# Teste Normale
FILE_TEST_DATE_1="checker/date/t%d/d.in"
FILE_TEST_CERINTE="checker/date/t%d/c.in"
FILE_TEST_REF="checker/rezultate/r%d.out"
FILE_TEST_OUT="checker/out/out%d.out"
NUM_TESTS=15
...
function run_test {
	...
	valgrind --leak-check=full --show-leak-kinds=definite --trace-children=no ./$EXEC_NAME $2 $3 $4 2>&1 | tee valgrind.log > /dev/null

	allocs=`grep "definitely lost:.*bytes" valgrind.log`
	frees=`grep "freed:.*bytes" valgrind.log`
